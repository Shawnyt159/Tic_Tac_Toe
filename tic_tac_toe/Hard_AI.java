...
public class Hard_AI extends AI{
	private char player;
	private boolean first_move = true;
	public Hard_AI(char player) {
		this.player = player;
	}
	
	public int[] Make_Move(char[][] board){
		if(this.first_move == true) {
			if(First_Move_Corner(board)) {
	...
	private boolean First_Move_Corner(char[][] board) {
		if(board[0][0] == this.player || board[0][2] == this.player || board[2][0] == this.player || board[2][2] == this.player) {
			return true;
		}
		return false;
	}
